...
{
	// 10 sheep will randomly settle in 4 pens. There must be at least 1 sheep in the pen
	public function welcome()
	{
		...
		// If the pens are empty, randomly settles the sheep
		if ( Sheep::isPaddockEmpty() ) {

			DB::beginTransaction();

			while ( $total > 0 ) {
				// randomly choose which pen to house one sheep
				$number = mt_rand(0, 3);
				$paddock[$number] += 1;
				$total -= 1;
			}
